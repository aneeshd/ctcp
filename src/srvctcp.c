...
main (int argc, char** argv){
    ...
    while(1){
        ...
        if((numbytes = recvfrom(sockfd, file_name, 1024, 0, &cli_addr, &clilen)) == -1){
...
send_segs(socket_t sockfd){
    ...
    if (dof_remain_cache < dof_needed){
        ...
        job->dof_request = MIN_DOF_REQUEST + dof_needed - dof_remain_cache;
...
coding_job(void *a){
    ...
    if (dof_request > 0){
      ...
      int q_remain = q_remaining(&coded_q[blockno%NUM_BLOCKS]);
      if (dof_request > q_remain){
        printf("postponing job... blockno %d, dof_request %d, q_remain %d\n", blockno, dof_request, q_remain);
        coding_job_t* job_postponed = malloc(sizeof(coding_job_t));
        job_postponed->blockno = blockno;
        job_postponed->dof_request = dof_request - q_remain;
        addJob(&workers, &coding_job, job_postponed, &free, LOW); 
        dof_request = q_remain;
      }
