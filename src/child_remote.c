...
int handle_con(int ctcp_port)
{
  ...
  */
  ...
  close(sk_target);
  printf("returned from sk_target close...\n");
  close_srvctcp(ctcp_sk);
  printf("returned from ctcp_srv close...\n");
  // CLOSE CTCP_SRV
  return ERR_NONE;
}
...
int negotiate( struct sockaddr *ad_cl_local, int ctcp_port )
{
  ...
  */
  buf[0] = SOCKS_VER;
  buf[1] = SOCKS_METHOD;
  sprintf(buf+2, "%d", ctcp_port);
  if( (res=writenb(sk_client,buf,7)) != ERR_NONE )
    return ERR_WRITE;
  ...
  while( 1 ) {
    ...
    */
    ...
    if( bptr < btop ) {
      ...
      if (bown == 0){ // if the proxy_local has sent something to remote forward over TCP
