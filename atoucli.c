...
int doit(char* host){
  ...
  socket_t	fd;			/* network file descriptor */
	...
	getsockopt(fd,SOL_SOCKET,SO_SNDBUF,(char *) &sndbuf,&i);
        setsockopt(fd,SOL_SOCKET,SO_RCVBUF,(char *) &rcvbuf,i);
        getsockopt(fd,SOL_SOCKET,SO_RCVBUF,(char *) &rcvbuf,&i);
	...
	while(snd_una < maxpkts){
		...
		if (r > 0) {  /* ack ready */
			...
			r=read(fd,buff,mss);
#else
			r= recvfrom(fd,buff,mss,0,&from,&fromlen);
#endif
			if (r <= 0) err_sys("read");
			rcvt = secs();
			ipkts++;
...
tcp_newreno(fd)
{
	...
	"packrxmit pkt %d nxt %d max %d cwnd %d  thresh %d recover %d una %d\n",
		...
		if (snd_cwnd < initsegs) snd_cwnd = initsegs;
		return 1;  /* yes was a partial ack */
	}
	return 0;
}
